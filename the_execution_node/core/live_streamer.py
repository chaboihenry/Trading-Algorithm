...
class LiveStreamer:
    ...
    def __init__(self, api_key: str, secret_key: str, base_url: str, models_dir: str = "the_models"):
        ...
        # 2. Lock for thread-safe matrix access
        # _handle_bar runs on asyncio thread, get_latest_matrix runs on main thread
        # Without this, pandas operations on live_matrix race and produce duplicate/skipped rows
        self._lock = threading.Lock()

        # 3. Load the universe immediately upon initialization (Will crash if missing)
        self._load_universe()

        # 4. Initialize the Alpaca Stream with Free Tier compliance
        self.stream = Stream(
            self.api_key,
            self.secret_key,
    ...
    async def _handle_bar(self, bar):
        ...
        # All matrix mutations protected by the lock to prevent races with reader thread
        with self._lock:
